...
### 修复
- **腾讯财经 ETF/指数前缀规则**：旧 `tencent_quote()` 只按首位数字判断市场，导致沪市 ETF（如 `510050`、`510300`）被错误请求为 `sz510xxx`，返回空结果；同时裸 `000001`/`000300` 指数示例和深市个股存在歧义。新增 `_tencent_prefix()`，支持显式 `sh/sz/bj` 前缀、`.SH/.SZ/.BJ` 后缀，并对沪市 ETF/指数、深市 ETF/指数、北交所代码做正确推断。

### 测试
- 腾讯财经真实接口验证：`tencent_quote(["510300"])` 返回沪深300ETF；`tencent_quote(["510050", "159915", "000001", "000300", "399006", "sz000001"])` 均能解析到正确市场；同批同时查询 `000001` / `sz000001` 时保留两条记录，避免纯 6 位代码 key 覆盖。
